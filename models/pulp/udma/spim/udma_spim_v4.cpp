...
Spim_periph_v4::Spim_periph_v4(udma *top, int id, int itf_id) : Udma_periph(top, id)
{
  std::string itf_name = "spim" + std::to_string(itf_id);

  channel0 = new Spim_v4_rx_channel(top, this, itf_name + "_rx");
  channel1 = new Spim_v4_tx_channel(top, this, itf_name + "_tx");
  channel2 = new Spim_v4_cmd_channel(top, this, itf_name + "_cmd");

  qspim_itf.set_sync_meth(&Spim_periph_v4::slave_sync);
  ...
  pending_spi_word_event = top->event_new(this, Spim_periph_v4::handle_spi_pending_word);
}

void Spim_periph_v4::reset(bool active)
{
...
void Spim_periph_v4::check_state()
{
  ...
  (static_cast<Spim_v4_tx_channel *>(this->channel1))->check_state();
  (static_cast<Spim_v4_cmd_channel *>(this->channel2))->check_state();
}
...
Spim_v4_tx_channel::Spim_v4_tx_channel(udma *top, Spim_periph_v4 *periph, string name)
: Udma_tx_channel(top, name), periph(periph)
{
  pending_word_event = top->event_new(this, Spim_v4_tx_channel::handle_pending_word);
}



void Spim_v4_tx_channel::handle_pending_word(void *__this, vp::clock_event *event)
{
  Spim_v4_tx_channel *_this = (Spim_v4_tx_channel *)__this;
...
void Spim_v4_tx_channel::handle_ready_reqs()
{
...
void Spim_v4_tx_channel::handle_data(uint32_t data)
{
...
void Spim_v4_tx_channel::reset(bool active)
{
...
void Spim_v4_tx_channel::check_state()
{
...
Spim_v4_cmd_channel::Spim_v4_cmd_channel(udma *top, Spim_periph_v4 *periph, string name)
: Udma_tx_channel(top, name), periph(periph)
{
  pending_word_event = top->event_new(this, Spim_v4_cmd_channel::handle_pending_word);
}


void Spim_v4_cmd_channel::handle_ready_reqs()
{
  if (!this->has_tx_pending_word && !ready_reqs->is_empty())
  {
    vp::io_req *req = this->ready_reqs->pop();
...
void Spim_v4_cmd_channel::check_state()
{
...
void Spim_periph_v4::handle_spi_pending_word(void *__this, vp::clock_event *event)
{
  Spim_periph_v4 *_this = (Spim_periph_v4 *)__this;
...
void Spim_v4_cmd_channel::handle_pending_word(void *__this, vp::clock_event *event)
{
  Spim_v4_cmd_channel *_this = (Spim_v4_cmd_channel *)__this;

  _this->handle_data(_this->tx_pending_word);
  _this->periph->check_state();
}

void Spim_v4_cmd_channel::handle_eot(bool cs_keep)
{
  if (!cs_keep)
  {
...
bool Spim_periph_v4::push_rx_to_spi(int nb_bits, int qpi, int lsb_first, int bitsword, int wordtrans)
{
...
bool Spim_periph_v4::push_tx_to_spi(uint32_t value, int nb_bits, int qpi, int lsb_first, int bitsword, int wordtrans)
{
  if (this->spi_tx_pending_bits == 0)
  {
...
void Spim_v4_cmd_channel::reset(bool active)
{
  Udma_tx_channel::reset(active);
...
void Spim_v4_rx_channel::reset(bool active)
{
