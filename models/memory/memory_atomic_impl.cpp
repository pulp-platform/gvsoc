...
#define MAX(a,b) (((a)>(b))?(a):(b))
  ...
  if (amo != vp::io_req_amo_e::NO_AMO) {
    ...
    // printf("Memory_atomic access (offset: 0x%x, size: 0x%x, is_write: %d, amo: %d)\n", offset, size, is_write, amo);
    ...
    // TODO handle sizes and signed correctly
    ...
    // for (int i=0; i<_this->res_table_size; i++) {
    //   if (_this->res_table[i] >= offset && _this->res_table[i] < offset+size) {
    //     _this->res_table[i] = -1;
    //   }
    // }
    for (auto& it : _this->res_table) {
