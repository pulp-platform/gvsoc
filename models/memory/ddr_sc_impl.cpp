...
SC_MODULE(ddr_module)
{

  friend class ddr;

  SC_HAS_PROCESS(ddr_module);
  ddr_module(sc_module_name nm, ddr *vp_module) : sc_module(nm), vp_module(vp_module)
  {
    SC_THREAD(run);
  }
...
void ddr_module::run()
{
  while(1)
  {
    ...
    wait(req->get_size() * 10, SC_NS);

    vp_module->current_reqs--;

    unsigned char *data = req->get_data();
