...
// Module able to buffer a second request before sending a response to the first
struct ems_target : sc_module {
    tlm_utils::simple_target_socket<ems_target> tsocket;

    SC_HAS_PROCESS(ems_target);
    ems_target(sc_module_name name, double accept_delay_ps, double internal_latency_ps, uint32_t bpa) :
        sc_module(name),
        tsocket("tsocket"),
    ...
    {
        accept_delay = sc_time(accept_delay_ps, SC_PS);
        internal_latency = sc_time(internal_latency_ps, SC_PS);
    ...
    tlm::tlm_sync_enum nb_transport_fw(tlm::tlm_generic_payload &p, tlm::tlm_phase &phase, sc_time &delay);
    void peq_callback(tlm::tlm_generic_payload &p, const tlm::tlm_phase &phase);

    sc_time accept_delay;
    sc_time internal_latency;
...
void ems_target::send_begin_resp(tlm::tlm_generic_payload &p)
{
    ...
    switch (status) {
        case tlm::TLM_UPDATED:
            ...
            // phase and delay updated by the callee
...
tlm::tlm_sync_enum ems_target::nb_transport_fw(tlm::tlm_generic_payload &p, tlm::tlm_phase &phase, sc_time &delay)
{
    ...
    switch (phase) {
        case tlm::BEGIN_REQ:
            if (!req_in_progress) {
                ...
                // Put back-pressure on initiator by
                // deferring END_REQ until pipeline is
                // clear
                end_req_pending = &p;
            }
            break;
        case tlm::END_RESP:
            // On receiving END_RESP, the target can
            // release the transaction and allow other
            // pending transactions to proceed
            if (!resp_in_progress) {
            ...
            req_in_progress = NULL;
            // ems_target itself is now clear to issue the
            // next BEGIN_RESP
            resp_in_progress = false;
            ...
            // and to unblock the initiator by issuing
            // END_REQ
            if (end_req_pending) {
